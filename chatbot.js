document.addEventListener('DOMContentLoaded', () => {
    ...
    const geminiChatAi = async (history) => { /* ... fungsi ini tidak berubah ... */ 
    ...
    // --- BARU: LOGIKA UNTUK DIKTE SUARA (VOICE-TO-TEXT) ---
    const setupSpeechRecognition = () => {
        const SpeechRecognition = window.SpeechRecognition || window.webkitSpeechRecognition;
        if (!SpeechRecognition) {
            micBtn.style.display = 'none'; // Sembunyikan tombol jika browser tidak mendukung
    ...
    micBtn.addEventListener('click', () => {
        if (recognition && !micBtn.classList.contains('mic-btn-recording')) {
            recognition.start();
        }
    });
    ...
    setupSpeechRecognition(); // Panggil fungsi setup untuk fitur suara
    startNewChat();
});
